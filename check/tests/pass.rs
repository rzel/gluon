...
";
    let result = support::typecheck(text);
    let variants = Type::variants(collect![(support::intern_unscoped("None"), support::typ_a("Option", vec![typ("a")])),
...
"#;
    ...
    let test = Type::variants(collect![(intern("Test"), variant)]);
    let types = vec![Field {
